...
import { AIDebugger } from './render/debugger.js';
...
const track=new Track(showcase.track==='harbor-ring'?'harbor-ring':null),session=new Session(track,{classId:showcase.classId,mixed:showcase.mixed}),world=new World(scene,renderer,track);
...
const aiDebugger=new AIDebugger(scene,session,()=>{cameraIndex=cameraIndex===4?0:4;setCamera(true);});
const input=new Keyboard(code=>{
  ...
  if(code==='F8'){
    ...
    if(photoFrozen){
      ...
      const photo=document.createElement('img');photo.id='photo-frame';photo.alt='Frozen Astra 3D scene';photo.src=renderer.domElement.toDataURL('image/png');
...
if(showcase.enabled){
  ...
  session.player.name='ASTRA AI';quality('performance');audio.setVolume(0);
  aiDebugger.panel.querySelector('#debug-car option').textContent='ASTRA AI';
...
window.astra={snapshot:()=>({phase:session.phase,time:session.time,mode:session.mode,autopilot:session.autopilot,trackLength:track.length,speed:session.player.speed*3.6,position:{x:session.player.x,z:session.player.z},lap:session.player.race.lap,contacts:session.contacts,physicsHz:120,drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles,fps,tyres:session.player.wheels.map(w=>({temperature:w.tyre.core,pressure:w.tyre.pressure,wear:w.tyre.wear})),cars:session.activeCars.map(c=>({name:c.name,speed:c.speed,s:c.s,lateral:c.lateral,state:session.drivers[c.id].state}))})};
function frame(now){
