...
export class UI {
  constructor(session,handlers){
    ...
    <header id="header"><a class="brand" href="#" aria-label="Astra home">${logo}<span>ASTRA<small>MOTORSPORT</small></span></a>
    ...
    <main id="menu">
      <section class="menu-panel" id="session-panel">
        ...
        <div class="circuit-card"><div class="circuit-heading"><div><small>YOUR NEXT CHALLENGE</small><h2>Circuit Solenne <span>↗</span></h2><p>Provence, France <span>·</span> Grand Prix layout</p></div><canvas id="menu-map" width="230" height="142" aria-label="Circuit Solenne track map"></canvas></div>
      ...
      <div class="vehicle-caption"><div class="eyebrow">ASTRA WORKS TEAM <span> / </span> 07</div><div class="vehicle-name">A1 <em>GT</em></div><div class="vehicle-specs"><span>575 <small>NM</small></span><span>1,290 <small>KG DRY</small></span><span>RWD <small>DRIVETRAIN</small></span></div><p>Engineered for the edge.</p></div>
      <div class="scene-marker"><span class="live-dot"></span> LIVE 3D <span> / </span> CIRCUIT SOLENNE</div>
    </main>
    ...
    <div id="hud" hidden>
      <div class="race-top"><div class="race-brand">${logo}<span>ASTRA <b>LIVE</b></span></div><div class="race-phase"><span class="live-dot"></span><span id="phase-label">SPRINT RACE</span></div><div class="race-actions"><button id="debug-toggle">RACE ENGINEER <small>B</small></button><button id="pause" aria-label="Pause race">Ⅱ <small>ESC</small></button></div></div>
    ...
    <div id="pause-overlay" class="overlay" hidden><section class="dialog"><span class="eyebrow">ASTRA MOTORSPORT</span><h2 id="pause-title">Take a breath.</h2><p id="pause-text">Your session is paused.</p><button class="primary" id="resume">RESUME SESSION ${arrow}</button><button class="secondary" id="restart">RESTART RACE</button><button class="text-button" id="exit">RETURN TO PADDOCK</button></section></div>
    ...
    $('.circuit-heading p').textContent=session.track.id==='harbor-ring'?'Waterfront circuit · exact Harbor Ring layout':'Provence, France · Grand Prix layout';
    $('#menu-map').setAttribute('aria-label',session.track.name+' track map');
    $('.vehicle-name').textContent='ASTRA '+spec.label;
    ...
    selection.innerHTML='<label>CIRCUIT<select id="circuit-choice"><option value="solenne">Circuit Solenne</option><option value="harbor-ring">Harbor Ring</option></select></label><label>YOUR CAR CLASS<select id="class-choice"><option value="gt">GT · rear-wheel drive</option><option value="touring">Touring · front-wheel drive</option><option value="prototype">Prototype · high downforce</option></select></label><label>OPPONENT CLASSES<select id="grid-choice"><option value="single">Same class as you</option><option value="mixed">Mixed · all three classes</option></select></label>';
