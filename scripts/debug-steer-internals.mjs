...
for (let tick = 0; tick < 300; tick++) {
  ...
  if (tick >= 100 && tick <= 240 && tick % 10 === 0) {
    const d = pilot.driver;
    const p = d._pNow;
    const eq = p.q - shadow.q;
    const psiPath = shadow.trackHeading - Math.atan2(p.dqds, Math.max(0.2, 1 - 0 * p.q));
    const beta = Math.atan2(shadow.v, shadow.u);
    const course = shadow.yaw - beta;
    let eHead = -(course - psiPath);
    console.log(`t=${(tick*DT).toFixed(2)}s | s=${shadow.s.toFixed(1)} | q=${shadow.q.toFixed(2)} | targetQ=${p.q.toFixed(2)} | eq=${eq.toFixed(2)} | yaw=${shadow.yaw.toFixed(3)} | course=${course.toFixed(3)} | eHead=${eHead.toFixed(3)} | pilotSteer=${pilot.controls.steer.toFixed(3)} | mode=${pilot.mode} | d.mode=${d.mode}`);
  }
}
