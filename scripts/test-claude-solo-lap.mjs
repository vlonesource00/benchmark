...
console.log('--- STARTING CLAUDE SOLO TEST ---');
for (let tick = 0; tick < 7200; tick++) {
  ...
  if (tick % 120 === 0) {
    console.log(`t=${(tick*DT).toFixed(1)}s | s=${shadow.s.toFixed(1)} | q=${shadow.q.toFixed(2)} | spd=${(vehicle.speed*3.6).toFixed(1)}km/h | str=${vehicle.controls.steer.toFixed(2)} | thr=${vehicle.controls.throttle.toFixed(2)} | brk=${vehicle.controls.brake.toFixed(2)} | mode=${pilot.mode}`);
  }
}
