...
console.log('--- STARTING 4-LAP GRAND PRIX TEST ---');
for (let tick = 0; tick < 28800; tick++) {
  race.step(DT);
  ...
  if (tick % 1200 === 0) {
    ...
    console.log(`t=${(tick*DT).toFixed(1)}s | Standings: ${standings.map(s => `${s.position}. ${s.name} (L${s.lap})`).join(', ')} | Claude s=${shadowByHostId.get('claude-racing').s.toFixed(1)} q=${shadowByHostId.get('claude-racing').q.toFixed(2)} spd=${(vehicles[1].speed*3.6).toFixed(1)}km/h mode=${cloudPilot.mode}`);
  }

  if (race.phase === 'complete') {
    console.log(`>>> RACE COMPLETE AT ${(tick*DT).toFixed(2)}s! Finish order: ${race.finishOrder.map(v => v.name).join(', ')}`);
    break;
  }
}
