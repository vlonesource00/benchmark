...
for (let tick = 0; tick < 3600; tick++) {
  ...
  if (tick % 60 === 0) {
    ...
    console.log(`t=${(tick*DT).toFixed(1)}s | s=${shadow.s.toFixed(1)} | q=${shadow.q.toFixed(2)} | spd=${(vehicle.speed*3.6).toFixed(1)}km/h | thr=${vehicle.controls.throttle.toFixed(2)} | brk=${vehicle.controls.brake.toFixed(2)} | str=${vehicle.controls.steer.toFixed(2)} | vAllow=${(d.vAllow*3.6)?.toFixed(1)}km/h | mode=${pilot.mode}`);
  }
}
