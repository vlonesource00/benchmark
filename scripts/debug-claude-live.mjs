...
// Race starts in grid/countdown then green
...
for (let tick = 0; tick < 1800; tick++) {
  ...
  if (tick % 120 === 0) {
    ...
    const n = vehicles[2];
    const gp = vehicles[3];
    console.log(`Tick ${tick} (${(tick*DT).toFixed(1)}s, phase=${race.phase}) | CLAUDE: s=${c.distance.toFixed(1)}, spd=${(c.speed*3.6).toFixed(1)}km/h, thr=${c.controls.throttle.toFixed(2)}, brk=${c.controls.brake.toFixed(2)}, str=${c.controls.steer.toFixed(2)}, mode=${cloudPilot.mode}, rsn="${cloudPilot.reason}", blocked=${cloudPilot.lattice?.blockedNow} | GPT spd=${(g.speed*3.6).toFixed(1)}, NMPCC spd=${(n.speed*3.6).toFixed(1)}, GP spd=${(gp.speed*3.6).toFixed(1)}`);
  }
}
