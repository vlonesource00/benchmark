...
for (let tick = 0; tick < 14400; tick++) {
  ...
  if (tick % 1200 === 0) {
    ...
    console.log(`Time ${(tick*DT).toFixed(1)}s (tick ${tick}) | Standings: ${standings.map(s => `${s.position}. ${s.name} (L${s.lap})`).join(', ')} | Claude spd: ${(vehicles[1].speed*3.6).toFixed(1)} km/h`);
  }
